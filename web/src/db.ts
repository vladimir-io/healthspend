...
export type SearchResponse = {
  ...
  statsQuery?: { sql: string; params: any[] };
};
...
export async function enrichSearchStats(
  statsQuery: { sql: string; params: any[] },
  limit: number,
  offset: number,
  rowCount: number
...
export async function searchPricesWithMeta(
  ...
  const deferStats = options.deferStats ?? DEFER_SEARCH_STATS_DEFAULT;
  const maxFallbackRounds = options.maxFallbackRounds ?? MAX_SEARCH_FALLBACK_ROUNDS;
  const cacheKey = `${query}:${state}:${zip}:${minConfidence.toFixed(2)}:${sort}:${limit}:${offset}:${deferStats}`;
  ...
  const canFallback = () => fallbackRounds < maxFallbackRounds;
  const recordFallbackRound = (reason: FallbackReason) => {
    fallbackRounds += 1;
    fallbacksUsed.push(reason);
    recordRum({ name: 'fallback', ms: 0, meta: { reason, round: fallbackRounds } });
  };
  ...
  if (results.length === 0 && state && canFallback()) {
    recordFallbackRound('state_fuzzy');
  ...
  if (results.length === 0 && (state || zip) && canFallback()) {
    recordFallbackRound('national_scope');
    const b = (ft: boolean) =>
      buildQuery(
        normQuery,
        '',
  ...
  if (results.length === 0 && zip && canFallback()) {
    recordFallbackRound('zip_relaxed_national');
    const b = (ft: boolean) =>
      buildQuery(
        normQuery,
        '',
  ...
  if (results.length === 0 && canFallback()) {
    recordFallbackRound('national_text_match');
  ...
  if (results.length === 0 && canFallback()) {
    recordFallbackRound('category_fallback');
  ...
  let statsQuery: { sql: string; params: any[] } | undefined;
  ...
  if (!finalSql) {
    ...
    statsPending = true;
    statsQuery = { sql: finalSql, params: finalParams };
  } else {
  ...
  const response: SearchResponse = {
    ...
    diagnostics: {
      ...
      fallbackRounds,
      fallbacksUsed,
      statsDeferred: deferStats && !!statsPending,
    },
    statsPending,
    statsQuery,
  };
